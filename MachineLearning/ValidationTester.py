...
def testModel(dataPath, meta, model):
    ...
    """
    xTrain, yTrain = getData(dataPath, "Train") #Getting the training data
    xVal, yVal = getData(dataPath, "Validation") #Getting the validation data
    ...
    pd.DataFrame({"yVal": yVal, "yPred": yPred}).to_csv(f"{meta["filepath"]}_Preds.csv", index=False) # Saving the data to a csv file

    dualPlot(yVal, yPred, meta) # Plotting the data (MSE hist + yhat v ytrue scatter plot)

    return yPred, yVal

def getData(dataPath, targ):
    data = pd.read_csv(f"{dataPath}-{targ}Set.csv")
...
if __name__ == "__main__":
    ...
    meta = {"version" : "0.6.2", #Setting up metadata
    ...
    for i in tdset: #Iterating through the datasets
        ...
        for j in talg: #Iterating through the models
            ...
            dataPath = (f"Data/Datasets/ScaledPredictionDatasets/{meta['version']}-{meta['dataset']}-{meta["variant"]}") #Reading in the dataset
